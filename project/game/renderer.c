...
void board_draw(board_t *b, bool hide_ships) {
  ...
  for (uint8_t row = 0; row < BOARD_ROWS; row++) {
    for (uint8_t col = 0; col < BOARD_COLS; col++) {
      if (b->grid[row][col] == CELL_MISS) {
        draw_cell(col, row, C_MISS); 
      } else {
        draw_cell(col, row, C_EMPTY); 
      }
    }
  }
...
void draw_hud_place(int player, int ship_idx) {
  ...
  if (ship_idx < NUM_SHIPS) {
    ...
    sprite_draw_rotated(spr_ships[ship_idx], 490, 555, true);
  }
